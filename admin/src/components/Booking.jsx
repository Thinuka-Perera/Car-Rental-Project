...
const StatusIndicator = ({ status, isEditing, newStatus, onStatusChange }) => {
  const display = String(status || "unknown");
...
const BookingCardHeader = ({ booking, onToggleDetails, isExpanded }) => (
  <div className={BookingPageStyles.bookingCardHeader} onClick={onToggleDetails}>
    ...
    <div>
      ...
      <div className={BookingPageStyles.bookingEmail}>{booking.email || ""}</div>
    </div>
    ...
    <div className={BookingPageStyles.bookingExpandIcon}>
      ...
      />
      <span className="ml-2 text-sm">{isExpanded ? "Hide Details" : "Show Details"}</span>
    </div>
  </div>
);
...
const BookingCardInfo = ({ booking, isEditing, newStatus, onStatusChange }) => (
  <div className={BookingPageStyles.bookingInfoGrid}>
    <div className="text-center">
      ...
      <div className={BookingPageStyles.bookingInfoValue}>{booking.car || ""}</div>
    </div>
...
const BookingCardActions = ({
  ...
  <div className={BookingPageStyles.bookingActions}>
    <div
      className="items-center text-orange-400 hidden md:flex cursor-pointer"
...
const BookingCardDetails = ({ booking }) => (
  <div className={BookingPageStyles.bookingDetails}>
    <div className={BookingPageStyles.bookingDetailsGrid}>
      ...
      <Panel title="Car Details" icon={<FaCar className={BookingPageStyles.panelIcon} />}>
        <div className="flex items-center mb-4">
          ...
          <div className="ml-4">
            <div className={BookingPageStyles.bookingCustomer}>{booking.car || ""}</div>
          </div>
        </div>

        <div className="grid grid-cols-2 gap-4">
          <Spec icon={<FaUserFriends />} label="Seats" value={booking.details.seats} />
          <Spec icon={<FaGasPump />} label="Fuel" value={booking.details.fuel} />
          <Spec icon={<FaTachometerAlt />} label="Mileage" value={booking.details.mileage} />
          <Spec icon={<FaCheckCircle />} label="Transmission" value={booking.details.transmission} />
        </div>
      </Panel>
    </div>
  </div>
...
const SearchFilterBar = ({
  ...
  <div className={BookingPageStyles.searchFilterContainer}>
    <div className={BookingPageStyles.searchFilterGrid}>
      ...
      <div>
        <label className={BookingPageStyles.filterLabel}>Filter by Status</label>
        <div className="relative">
          <select
            value={statusFilter}
            onChange={onStatusChange}
      ...
      <div className={BookingPageStyles.totalBookingsContainer}>
        <div className="text-center">
          <div className={BookingPageStyles.totalBookingsLabel}>Total Bookings</div>
          <div className={BookingPageStyles.totalBookingsValue}>{totalBookings}</div>
        </div>
      </div>
    </div>
  </div>
...
const Booking = () => {
  ...
  const filteredBookings = useMemo(() => {
    ...
    return bookings.filter((b) => {
      const matchesSearch =
        s(b.customer).includes(q) ||
        s(b.car).includes(q) ||
        s(b.email).includes(q);
  ...
  const updateStatus = async (id) => {
    try {
      ...
      if (!newStatus) {
        window.alert("Please select a status before updating.");
        return;
      }
      ...
      <div className="space-y-4">
        {filteredBookings.map((booking) => (
          <BookingCard
            ...
            booking={booking}
            isExpanded={expandedBooking === (booking.id || booking._id)}
            isEditing={editingStatus === (booking.id || booking._id)}
            newStatus={newStatus}
            onToggleDetails={() =>
              handleToggleDetails(booking.id || booking._id)
            }
            onEditStatus={(e) => {
              e.stopPropagation();
              handleEditStatus(booking.id || booking._id, booking.status);
            }}
            onStatusChange={(e) => setNewStatus(e.target.value)}
            onSaveStatus={(e) => {
              e.stopPropagation();
              updateStatus(booking.id || booking._id);
            }}
            onCancelEdit={(e) => {
              e.stopPropagation();
              handleCancelEdit();
